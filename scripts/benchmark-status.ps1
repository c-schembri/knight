...
$work = Join-Path $root "target\differential-benchmark-status-$Edges"

cargo build --release --manifest-path (Join-Path $root 'Cargo.toml')
New-Item -ItemType Directory -Force $work | Out-Null

$manifest = [System.Text.StringBuilder]::new($Edges * 48)
[void]$manifest.Append("rule touch`n  command = cmd /d /c type nul > `$out`n")
for ($edge = 0; $edge -lt $Edges; $edge++) {
    [void]$manifest.Append("build out/$edge`: touch`n")
}
[void]$manifest.Append('build all: phony')
for ($edge = 0; $edge -lt $Edges; $edge++) {
    [void]$manifest.Append(" out/$edge")
}
...
function Invoke-DryRun([string]$Executable) {
    $startInfo = [System.Diagnostics.ProcessStartInfo]::new()
    $startInfo.FileName = $Executable
    $startInfo.WorkingDirectory = $work
    $startInfo.Arguments = '-n -j1 all'
...
$expected = (Invoke-DryRun $tools.ninja).Output
$actual = (Invoke-DryRun $tools.knight).Output
if ($actual -cne $expected) {
    throw 'Knight and Ninja produced different redirected dry-run status output'
}

foreach ($name in $tools.Keys) {
    for ($warmup = 0; $warmup -lt 3; $warmup++) {
        [void](Invoke-DryRun $tools[$name])
    }
}
for ($iteration = 0; $iteration -lt $Iterations; $iteration++) {
    $order = if ($iteration % 2 -eq 0) { @('ninja', 'knight') } else { @('knight', 'ninja') }
    foreach ($name in $order) {
        $samplesByTool[$name].Add((Invoke-DryRun $tools[$name]).ElapsedMs)
    }
}
