        ...
        if parse_options && let Some(value) = argument.strip_prefix("--builddir=") {
            ...
            if argument.starts_with("--") {
                ...
                let option = argument
                    ...
                    .unwrap_or('?');
                print_invalid_long_tool_option("restat", option);
            }
            return tool_restat_usage();
        } else {
...
fn print_invalid_long_tool_option(tool: &str, option: char) {
    #[cfg(any(
        target_os = "dragonfly",
        target_os = "freebsd",
        target_os = "macos",
    ...
    print_invalid_tool_option(tool, option);
}
        ...
        if parse_options && argument.starts_with("--") {
            ...
            while let Some((_, option)) = characters.next() {
                match option {
                    ...
                    _ => {
                        print_invalid_long_tool_option(
                            if grouped { "multi-inputs" } else { "inputs" },
                            option,
                        );
                        return tool_inputs_usage(grouped);
